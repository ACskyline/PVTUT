    ...
    */
    ...
    public class DynamicDecals : MonoBehaviour
    {
        ...
        private bool FireInCulling
        {
            get
            {
                return !(XRSettings.enabled || Settings.Replacement == ShaderReplacementType.VR);
            }
        }
        ...
        public static string DebugLog
        {
            get
            {
                ...
                if (XRSettings.enabled)
                {
                    ...
                    debug += "VR API : " + XRSettings.loadedDeviceName + "\r\n";
                    debug += "VR device : " + XRDevice.model + "\r\n";
    ...
    internal class CameraData
    {
        ...
        private void UpdateRenderTextures(Camera Camera, DynamicDecals System, bool ForceNewTextures = false)
        {
            ...
            if (VRCamera(Camera))
            {
                width = (System.Settings.SinglePassVR) ? XRSettings.eyeTextureWidth * 2 : XRSettings.eyeTextureWidth;
        ...
        private void GetTextures(Camera Camera, DynamicDecals System, int Width, int Height)
        {
            switch (replacement)
            {
                case ShaderReplacement.SingleTarget:
                    ...
                    if (VRCamera(Camera) && System.Settings.SinglePassVR) depthEye = RenderTexture.GetTemporary(XRSettings.eyeTextureWidth, XRSettings.eyeTextureHeight, 24, RenderTextureFormat.RGFloat);
                    break;
                ...
                case ShaderReplacement.TriplePass:
                    ...
                    {
                        if (Camera.actualRenderingPath == RenderingPath.DeferredShading) depthEye = RenderTexture.GetTemporary(XRSettings.eyeTextureWidth, XRSettings.eyeTextureHeight, 24, System.depthFormat);
            ...
            if (System.ShaderReplacement)
            {
                ...
                if (VRCamera(Source) && System.Settings.SinglePassVR)
                {
                    ...
                    if (Source.stereoTargetEye == StereoTargetEyeMask.Both || Source.stereoTargetEye == StereoTargetEyeMask.Left)
                    {
                        ...
                        {
                            Renderer.transform.position = Source.transform.parent.TransformPoint(InputTracking.GetLocalPosition(XRNode.LeftEye));
                        }
                        else
                        {
                            Renderer.transform.position = InputTracking.GetLocalPosition(XRNode.LeftEye);
                        }
                        
                        //Rotation
                        Renderer.transform.rotation = Source.transform.rotation * InputTracking.GetLocalRotation(XRNode.LeftEye);
                    ...
                    if (Source.stereoTargetEye == StereoTargetEyeMask.Both || Source.stereoTargetEye == StereoTargetEyeMask.Right)
                    {
                        ...
                        {
                            //Position
                            Renderer.transform.position = Source.transform.parent.TransformPoint(InputTracking.GetLocalPosition(XRNode.RightEye));
                        ...
                        {
                            //Position
                            Renderer.transform.position = InputTracking.GetLocalPosition(XRNode.RightEye);
                        ...
                        //Rotation
                        Renderer.transform.rotation = Source.transform.rotation * InputTracking.GetLocalRotation(XRNode.RightEye);
        ...
        private void SetupReplacementCameraExperimental(Camera Source, Camera Target)
        {
            ...
            //Fov and ranges
            if (!XRSettings.enabled) Target.fieldOfView = Source.fieldOfView;
