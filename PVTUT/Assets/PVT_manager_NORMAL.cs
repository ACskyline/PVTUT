...
public class PVT_manager_NORMAL : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.G))
        {
            for (int i = 0; i <= mipLevelMax; i++)
            {
                ...
                //}
                //while (temp != null);
                for(int j = 0;j<pageMipLevelTable[i].count;j++)
                {
                    if (pageMipLevelTable[i].chunks[j].decalMatrixList.Count > 0)
                    {
                        Debug.Log("mipLevel:" + i + "=>" + pageMipLevelTable[i].ChunkToXZ(j) + pageMipLevelTable[i].chunks[j].decalMatrixList.Count);
                    }
                }
            }
        }
    ...
    void InitializeTextureArray()
    {
        ...
        textureArrayArray = new RenderTexture[mipLevelMax + 1];
        normalArrayArray = new RenderTexture[mipLevelMax + 1];
        ...
        for (int i = 0; i <= mipLevelMax; i++)
        {
            ...
            textureArrayArray[i] = new RenderTexture(tileWidth, tileHeight, 16, RenderTextureFormat.ARGB32, RenderTextureReadWrite.sRGB);//16 bit depth buffer for sorting decals
            ...
            textureArrayArray[i].useMipMap = true;//default to false
            textureArrayArray[i].autoGenerateMips = false;
            textureArrayArray[i].filterMode = FilterMode.Trilinear;
            textureArrayArray[i].Create();

            normalArrayArray[i] = new RenderTexture(tileWidth, tileHeight, 16, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);//16 bit depth buffer for sorting decals
