...
A side note to this topic is an upgrade of the current LOD->Mipmap decision can be achieved by using CDLOD where the proper LOD of each chunk is decided by it's distance to eye than pure 2D distance. This is done by doing box-sphere intersection detection and a quad-tree traversal. This method will always assign appropriate LOD to each chunk. If a hill and a plain exist at the same area, the quad-tree traversal will go deeper in the chunk that still intersects the eye bounding sphere, no matter it's a hill or a plain. It is a more dynamic and uniform LOD decision algorithm(and corresponding data structure(the quad tree)) that works similarly as how Mipmaps are decided.
